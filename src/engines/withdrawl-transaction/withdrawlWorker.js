...
const splitTransaction = async (
    ...
    try {
        ...
        if (postTransfer.status === 200) {
            ...
            const htmlContent = htmlContentSuccess(transferBody.amount, transaction, route.target, balance);
...
const htmlContentSuccess = (amount, transaction, store, balance) => {
    const total = transaction.total_with_fee - transaction.quick_release_fee - transaction.quick_release_vat - transaction.fee_bank - transaction.vat;
